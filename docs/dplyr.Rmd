...
4. Save output as a variable: `females_weight_above_30`
```{r save output}
females_weight_above_30 <- filter(data, weight_in_g > 30, sex == "F")
```

5. Display only `weight_in_g` of females with `weight_in_g > 30`
```{r select }
select(females_weight_above_30, weight_in_g)
```

6. Select everything from `females_weight_above_30` except device_remarks
```{r }
select(females_weight_above_30, -device_remark)
```
