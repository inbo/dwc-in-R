...
- check whether the class of the column values fits your purpose and in case apply changes by clicking on the class name below the column name,
...
* **head(df)** returns the first part of a data frame. It works with several other data types (vectors, tables, ...). Specifiy how many rows to display with parameter `n`: `head(df, n = )`, default `n = 6`
...
You can apply `mean()`, `min()` and `max()` to dataframes as well: the functions will return respectively mean, minimum and maximum of each column. However, in case one or more columns are not numeric, an error occurs. In case one or more `NA` are present in the data, you need to add `na.rm = TRUE` as parameter in the function call!
...
data %>% filter(!is.na(weight_in_g)) %>% 
  group_by(sex) %>% summarise(mean_weight = mean(weight_in_g))
```
