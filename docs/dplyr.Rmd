...
#> [10] "c"
```
You can specify a `default` value: it will be given to all values not otherwise matched. And if you want replace missing values (NA), then you can specify a value to the parameter `limit`.
...
### Exercise
1. Add column `scientific_name` with full scientific name based on the table below:

| species | scientific name                          |

| DM      | Dimarella riparia (Navás, 1918)          |
...
After hours of data analysis maybe you would like to save the output, isn't it?
 ...
 - `row.names`, a logical, TRUE or FALSE, whether you want or not row numbers or names (if specified in your dataframe) in the file
 - `col.names`, a logical, TRUE or FALSE, indicating the presence of column names in the first line of the file.
 - `append`, a logical, TURE or FALSE. Default value: FALSE. If the specified file already exists, it would be overwritten, while `append = TRUE` will append the dataframe at the end of the file.
 
### Exercise
1. Export `females_weight_above_30` to a tab delimited csv file in
../data/processed/females_weight_above_30.csv with column names but no row names/indexes.
```{r}
