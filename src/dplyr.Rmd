...
It is normally useful to sort data. The package `dplyr` offers the function `arrange()` to do it. If no columns are specified, the dataframe is sorted by values of the first column (`arrange(df)`). Use `desc()` to sort in descending order.
...
You would like to calculate the sum for each category of items. We can do it via `%>%` and two functions from dplyr library: `group_by()` and `summarize()`:
...
If you want to count the number of rows with each unique value of a column variable, you can use the function `count` instead of `summarize`:
...
`../data/processed/data_scientificname.csv`. Add column names and row names/indexes
