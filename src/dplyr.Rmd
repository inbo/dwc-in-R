...
Before to start manipulate data, it is good practice to explore the data in order to check whether everything is correctly imported. Moreover, it gives an idea about the data we are going to analyse/manipulate. R gives us several tools to do it:

1. **`head(df)`** returns the first part of a data frame. It works with several other data types (vectors, tables, ...). Specifiy how many rows to display with parameter `n`: `haed(df, n = )`, default `n = 6`
2. **`View(df)`** opens the entire dataframe in viewer. If dataframe is very big, then your computer can slow down sensibly and Rstudio can eventually crash. Be aware of it!
3. **`class(df)`** if `df` is a dataframe then `"data.frame"` will show up
4. **`str(df)`** displays the internal structure of the dataframe
5. **`colnames(df)`** display column names
6. **`nrow(df)`** returns the number of rows of the dataframe
7. **`ncol(df)`** return the number of rows of the dataframe
9. **`max(df)`** returns the maximum of all elements of a (numeric) dataframe
10. **`min(df)`** returns the minimum of all elements of a (numeric) dataframe

While applying `max()` or `min()` an error will occur if one or more `NA` are present. What you need is just adding `na.rm = TRUE` as parameter in the function call.
...
You can select specific columns of a dataframe by `select()`. Only columns where conditions are true are selected. For example, selecting a column by its column name: `select(df, column_name)`. To select all columns except a specific column use the minus symbol before its name: `select(df, -column_name)`.

To select rows use `filter()`. Again, one or multiple filters can be applied to filter the targeted rows: `filter(df, condition1, condition2,...)`. A condition can be any logical operator (`>`, `>`, `<=`,...). For example, to get only those rows for which the `weight_in_g` is smaller than 40, use `filter(data, weight_in_g < 40)`.
...
```{r pipes4}
females_weight_above_30 <- data %>% filter(sex == "F", weight_in_g > 30) %>%
  select(weight_in_g)
```
...
5. Show minimum, maximum and mean weight per sex and species and save output as `weight_per_species_sex`
